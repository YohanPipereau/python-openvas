...
def ListFamilies(oidTimeout,initialize_timer):
    ...
    sock = SocketConnect.SocketConnect(oidTimeout,initialize_timer) #outputVar is the answer of scanner to message
    sock.Send(message)
    outputVar = sock.Receive() 
...
def RunScan(oidTimeout,ipScan,verbose,oidList,initialize_timer):
    """
        Called from the main file.
    ...
    sock = SocketConnect.SocketConnect(oidTimeout,initialize_timer)
    sock.Send(message)
    outputScan = sock.Receive(verbose)
    return(outputScan)
